...
contract Treasury is Ownable {
	...
	//There can  be a general purpose treasury for any 
	    constructor(address token_addr) Ownable(msg.sender) {
        token = IERC20(token_addr);
      
	    }

    function withdraw(uint256 _amount) public {
        require(msg.sender == yeildEngine, "Invalid sender address!!");
        token.safeTransfer(msg.sender, _amount);
        }
		
        function setUpOperator(address _operator) external onlyOwner {
            require(yeildEngine == address(0),"Only One update allowed!");
            yeildEngine=_operator;
        } 
